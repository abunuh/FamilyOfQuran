...
import { ScrollText, Heart, Share2, ChevronDown, Copy, Twitter, Facebook, MessageCircle, FileText } from 'lucide-react';
...
export default function HadithResultCard({ result, language }) {
  ...
  const hadithId = result.id || result.hadithReference;
  const bookmarked = isBookmarked(hadithId, 'hadith');

  const handleExpand = () => {
    const newExpanded = !isExpanded;
    setIsExpanded(newExpanded);
    if (newExpanded) {
      addToHistory(result, 'hadith');
    }
  };

  const handleBookmark = () => {
    setIsAnimatingHeart(true);
    const added = toggleBookmark(hadithId, 'hadith', result);
    if (added) {
      toast.success('Hadith added to bookmarks');
    } else {
  ...
  const handleAddToNotepad = () => {
    const text = `"${result.hadithText}"\n— ${result.collectionName} ${result.hadithNumber}`;
    appendToNotepad(text);
    toast.success('Added to Notepad');
  };

  const shareText = `"${result.hadithText}" - ${result.collectionName} ${result.hadithNumber} | Family of the Quran`;
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${
                  result.grade?.toLowerCase().includes('sahih') ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 
                  result.grade?.toLowerCase().includes('hasan') ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' : 
          ...
          <div className="flex items-center gap-2 self-end sm:self-auto">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuItem onClick={() => copyToClipboard(result.hadithText)} className="cursor-pointer">
                  <Copy className="w-4 h-4 mr-2" /> Copy Text
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => copyWithReference(result.hadithText, `${result.collectionName} ${result.hadithNumber}`)} className="cursor-pointer">
                ...
                <div className="pt-4 pb-2 space-y-4 text-sm">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="bg-muted/30 p-3 rounded-lg">
                      <span className="block text-muted-foreground mb-1">Narrator</span>
                      <span className="font-medium text-foreground">{result.narrator || 'Abu Huraira (RA)'}</span>
                    </div>
                    <div className="bg-muted/30 p-3 rounded-lg">
                      <span className="block text-muted-foreground mb-1">Authenticity</span>
                      <span className="font-medium text-foreground">{result.grade || 'Sahih (Authentic)'}</span>
                    </div>
                  </div>
                  {result.translation && (
